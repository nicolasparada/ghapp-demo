...
async function stopAgentIfRunning() {
    ...
    if (gracefulResult.processExited === false) {
        if (gracefulResult.summaryFileReady) {
            log("INFO", `summary file appeared before process exit; skipping SIGKILL to avoid truncating output (pid=${pid})`);
        }
        else {
...
async function emitAgentLog(path, stream) {
    ...
    try {
        ...
        log("INFO", `emitting agent ${stream} log tail (${tailLines.length}/${allLines.length} lines) from ${path}`);
...
async function waitForProcessExitOrSummary(pid, summaryPath, timeoutMs) {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
...
function isErrnoWithCode(error) {
...
async function uploadSummary(controlPlaneBaseUrl, summaryBytes) {
...
function isUploadTokenResponse(value) {
    if (typeof value !== "object" || value === null) {
        return false;
    }
    const record = value;
