...
async function waitForAgentReadiness(args) {
    ...
    while (Date.now() < deadline) {
        ...
        if (exited) {
            ...
            log("WARN", `agent exited before readiness probe succeeded (pid=${args.pid}); egress capture may be unavailable`);
            if (stderrTail !== "") {
    ...
    const stderrTail = await readLogTail(args.stderrLogPath, AGENT_LOG_TAIL_LINES);
    log("WARN", `agent readiness timed out after ${AGENT_READY_TIMEOUT_MS}ms; continuing fail-open`);
    if (stderrTail !== "") {
        log("WARN", `agent stderr tail while waiting readiness:\n${stderrTail}`);
    }
}
function didProcessExit(pid) {
    try {
        process.kill(pid, 0);
        return false;
    }
